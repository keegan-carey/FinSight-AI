...
export const DEFAULT_PRIVACY_SETTINGS: PrivacySettings = {
  dataCollection: true, shareAnalytics: true, personalizedAds: false,
  thirdPartySharing: false, retentionPeriod: '1year', exportFormat: 'json',
...
export async function updatePrivacySettings(userId: string, settings: Partial<PrivacySettings>): Promise<void> {
  try {
    await setDoc(doc(db, 'privacy_settings', userId), { ...settings, lastUpdated: new Date().toISOString() }, { merge: true });
  } catch { handleFirestoreError(settings, OperationType.UPDATE, 'privacy_settings'); }
}

export async function exportUserData(userId: string): Promise<Record<string, any>> {
  const data: Record<string, any> = {};
  for (const colName of ['transactions', 'subscriptions', 'anomalies', 'reports', 'trend_analysis']) {
    try {
      const snap = await getDocs(query(collection(db, colName), where('userId', '==', userId)));
      data[colName] = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    } catch { data[colName] = []; }
  }
  try { data.profile = (await getDoc(doc(db, 'users', userId))).data(); } catch {}
  return data;
}

export async function deleteUserData(userId: string): Promise<void> {
  const batch = writeBatch(db);
  for (const colName of ['transactions', 'subscriptions', 'anomalies', 'reports', 'trend_analysis', 'goals']) {
    try {
      (await getDocs(query(collection(db, colName), where('userId', '==', userId)))).docs.forEach((d) => batch.delete(d.ref));
    } catch {}
  }
  try {
    ...
    await batch.commit();
  } catch (error) { handleFirestoreError(error, OperationType.DELETE, 'userData'); }
}
