...
import { ShieldCheck, Loader2, Download, Trash2, LogOut, Globe, Lock, Eye, Activity } from 'lucide-react';
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-white leading-none">Privacy Dashboard</h1>
            <p className="text-slate-500 text-sm mt-2">Manage your data and privacy settings</p>
          </div>
        </div>
        <Card className="bg-slate-900 border-slate-800 rounded-2xl">
          <CardContent className="p-8 flex flex-col items-center justify-center min-h-[400px]">
            <Loader2 className="h-8 w-8 animate-spin text-indigo-500" />
            <p className="text-sm font-medium text-slate-500 mt-4">Loading privacy settings...</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-12">
      <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-800 pb-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white leading-none">Privacy Dashboard</h1>
          <p className="text-slate-500 text-sm mt-2">Control your data, sessions, and privacy preferences</p>
        </div>
      </section>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="bg-slate-900 border-slate-800 rounded-2xl">
            <CardHeader className="p-5 border-b border-slate-800">
              <CardTitle className="text-sm font-bold uppercase tracking-wider text-white">Data Management</CardTitle>
              <CardDescription className="text-slate-500 text-xs">Export or delete your financial data</CardDescription>
            </CardHeader>
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-white">Export My Data</p>
              ...
              <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-white">Delete All Data</p>
          ...
          <Card className="bg-slate-900 border-slate-800 rounded-2xl">
            <CardHeader className="p-5 border-b border-slate-800">
              <CardTitle className="text-sm font-bold uppercase tracking-wider text-white">Privacy Preferences</CardTitle>
              <CardDescription className="text-slate-500 text-xs">Configure your data sharing and retention settings</CardDescription>
            </CardHeader>
            <CardContent className="p-5 space-y-4">
              {privacySettings && (
                <>
                  <PrivacyToggle
                    ...
                    onChange={(enabled) => setPrivacySettings({ ...privacySettings, dataRetentionEnabled: enabled })}
                  />
                  <PrivacyToggle
                    label="Usage Analytics"
                    description="Allow anonymous usage analytics to improve the service"
                    enabled={privacySettings.analyticsEnabled}
                    onChange={(enabled) => setPrivacySettings({ ...privacySettings, analyticsEnabled: enabled })}
                  />
                  <PrivacyToggle
                    label="Data Sharing"
                    description="Share anonymized data for research purposes"
                    enabled={privacySettings.sharingEnabled}
                    onChange={(enabled) => setPrivacySettings({ ...privacySettings, sharingEnabled: enabled })}
                  />
                </>
              )}
            </CardContent>
          ...
          <Card className="bg-slate-900 border-slate-800 rounded-2xl">
            <CardHeader className="p-5 border-b border-slate-800">
              <CardTitle className="text-sm font-bold uppercase tracking-wider text-white">Activity Log</CardTitle>
              <CardDescription className="text-slate-500 text-xs">Recent account activity</CardDescription>
            </CardHeader>
...
function PrivacyToggle({ label, description, enabled, onChange }: { label: string; description: string; enabled: boolean; onChange: (enabled: boolean) => void }) {
  return (
    <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl">
